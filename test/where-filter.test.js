...
context('whereFilter', () => {
	...
	describe('wildcards and metacharacters', () => {
		...
		it('should match verbatim regular expression metacharacters', () => {
			...
			assert.equal(
				whereFilter({ x: { like: escapedMetachars } })({ x: metachars.replace('[', 'x') }),
				false,
			);
	...
	it('matches values in data array', () => {
		...
		assert.isTrue(
			whereFilter({ or: [{ colors: { neq: 'red' } }, { colors: { neq: 'white' } }] })(data),
		); // matched white
		...
		assert.isTrue(
			whereFilter({ and: [{ colors: { neq: 'red' } }, { colors: { neq: 'white' } }] })(data),
		);
	});
